 ...
 * All operations are non-blocking, utilizing {@link CompletableFuture} for asynchronous execution,
 * and are secured using Spring Security's method-level permissions.
 * </p>
 *
...
public class BranchController {
     ...
     * with transactionStatus 201 (Created) and the created {@link BranchResponse} body.
     */
    @Operation(summary = "Create a new bank branch entry")
    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    public CompletableFuture<ResponseEntity<BranchResponse>> createBranch(
     ...
     * @return A {@link CompletableFuture} that resolves to a {@link ResponseEntity}
     * with transactionStatus 200 (OK) and the {@link BranchResponse} body.
     */
    @Operation(summary = "Get a branch by ID")
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'EMPLOYER', 'ACCOUNTANT')")
    public CompletableFuture<ResponseEntity<BranchResponse>> getBranchById(
     ...
     * @return A {@link CompletableFuture} that resolves to a {@link ResponseEntity}
     * with transactionStatus 200 (OK) and the updated {@link BranchResponse} body.
     */
    @Operation(summary = "Update an existing bank branch")
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public CompletableFuture<ResponseEntity<BranchResponse>> updateBranch(
            @PathVariable UUID id,
     ...
     * @return A {@link CompletableFuture} that resolves to a {@link ResponseEntity}
     * with transactionStatus 204 (No Content) upon successful deletion.
     */
    @Operation(summary = "Delete a branch by ID")
    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @PreAuthorize("hasRole('ADMIN')")
    public CompletableFuture<ResponseEntity<Void>> deleteBranch(@PathVariable UUID id) {
     ...
     * @return A {@link CompletableFuture} that resolves to a {@link ResponseEntity}
     * with transactionStatus 200 (OK) and a {@link Page} of {@link BranchResponse} objects.
     */
    @Operation(summary = "Search branches using dynamic filters and pagination")
    @GetMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'EMPLOYER', 'ACCOUNTANT')")
    public CompletableFuture<ResponseEntity<Page<BranchResponse>>> searchBranches(
        ...
        return branchService.search(filter, pageable)
                .thenApply(ResponseEntity::ok);
    }
}
